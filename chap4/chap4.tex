...
an adaptive scheme. \red{Give details.} To discuss the resolution of the computational domain for the purpose of convergence tests, we will use the notation
\begin{equation}
...
desired value. \red{How does this actually work?}
...
$\chi_{\rm BH}=0.9$. \red{Harald says - ``define $M_{\rm ADM}$ up
  front''... not sure what he means.}The neutron star in each binary has
...
orbital parameters $M\Omega$ and $D/M$ are constant. \red{Imrpove the
  discussion below.}While not the
...
\red{Re-do plot to put a big open circle in the middle.}
...
different resolutions used in the initial data solve. \red{Expand explanation}In our notation, {\tt R1} is the first (lowest) resolution, {\tt R2} is the second, etc. In the figure we see that at a given resolution these differences
...
\red{Replace RN with $N^{1/3}\approx 33.0$ etc.}
...
it is no longer solved adjusted. \red{Explain why.} We note that in general we see a decrease in this difference with iteration, especially at the first resolution, therefore showing that the iterative solver is correctly driving the the black hole properties to the target values.
...
and mass. \red{Discuss sawtooth pattern in the figure.}

\begin{figure}
\includegraphics[width=0.95\columnwidth]{chap4/BHSpinConv}
\caption[Convergence of black hole spin and
mass.]{\label{Fig:BHSpinConv} \red{Make this plot less confusing based
    on Harald's suggestions.}Plotted is the fractional difference from the target black hole spin and black hole mass in the {\tt R14i60$\Uparrow$} initial data set as a function of iteration. The four colours represent 
...
neutron star surface is only computed for the first four resolutions \red{Why?},
...
\red{Move text from Fig4.2 here}We now turn to three more initial data sequences to help fill out the
...
worth discussing in the data. \red{Fix wording of next two sentences.}The size of the bump at {\tt R2} as noted
...
\red{Move 4.33 + discussion here.}As with the previous sequence, it is interesting to look at the
...
$q$.]{\label{fig:qchi}\red{Add little ticks on x-axis. Extend chimax
    to 1.}Neutron star spin $\chi_{\rm
